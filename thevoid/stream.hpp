...
private:
	...
	/*!
	 * \internal
	 * TODO: explain invariant, m_data before on_data call is not ready to be processed
	 */
	size_t on_data(const boost::asio::const_buffer &buffer)
	{
		auto begin = boost::asio::buffer_cast<const char *>(buffer);
		...
		// TODO: no reason to process the loop with empty buffer
		while (size) {
			// TODO: explain the meaning of real_chunk_size
			...
			// TODO: explain minus one in details
			...
			//// It can occur if buffer's len equals to one. That is we have one byte in buffer.
			//// We should not proccess it in order to be able to initiate a call of on_data later.
			// TODO: explain why delta == 0 means we should not continue the loop
			if (delta == 0) {
				break;
			}
			...
			if ((m_data.size() == m_chunk_size || m_data.size() == m_unprocessed_size)
					&& m_client_asked_chunk) {
				this->process_chunk_internal();
			}

		//// The loop is going until the chunk is not full and the buffer is not fully processed.
		}
